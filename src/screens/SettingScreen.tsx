  ...
  return (
    <TouchableOpacity onPress={props.onPress}>
      <View key={Math.random()} style={[
        ...
        {(props.icon)
          ? <TabBarIcon style={styles.iconStyle} focused={false} name={props.icon} iconColor={AppColor.white} size={18} />
          : null }
        <AppText>
...
export default function SettingScreen() {
  ...
  return (
    <View style={styles.container}>
      {
        isConnect
        ? <ListItem height={50} content={'Disconnect'} icon={'disconnect'}
            onPress={ async () => {
          ...
          />
        : <ListItem height={50} content={'Connect'} icon={'adduser'}
            onPress={ async () => {
